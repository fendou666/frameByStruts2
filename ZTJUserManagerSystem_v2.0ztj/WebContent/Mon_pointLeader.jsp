...
function getPage(forword){
	$.post(
		"MonitorInfoServlet?action=pointLeader&page="+forword+"&classId=${sessionScope.user.classId}",
		{  },
		function(data){
			...
			$.each(students,function(i){
				...
				htmStr+="<td>";
				htmStr+="<a href=\"MonChooseServlet?action=modifyLeader&id=${stu.id}&classId=${sessionScope.user.classId}&groupId=${stu.groupId}&headmasterId=${sessionScope.user.headmasterId}&teacherId=${sessionScope.user.teacherId}\" onclick=\"confrimUser()\">指定组长</a>"
				htmStr+="</td>";
				htmStr+="</tr>";	
			})
			htmStr+="<tr>";
				...
				<div class="main-title">
					...
					<div class="context">
						...
						<div class="con_right" style="margin-left: 205px;">
							...
							<div class="con_rmenu" id="nav">用户管理>>指定组长</div>
							<hr>
							<div class="con_rcon">
								<form action="MonChooseServlet" method="post">
									<table border="2" align="center" class="con_table">
										...
										<c:choose>
											<c:when test="${not empty requestScope.listStuEM}">
												<c:forEach items="${requestScope.listStuEM}" var="stu"
													varStatus="iid">
													<tr>
														...
														<td>
															<a href="MonChooseServlet?action=modifyLeader&id=${stu.id}&classId=${sessionScope.user.classId}&groupId=${stu.groupId}&headmasterId=${sessionScope.user.headmasterId}&teacherId=${sessionScope.user.teacherId}" onclick="confrimUser()">指定组长</a>
														</td>
													</tr>
												</c:forEach>
												<tr>
