...
public class StudentDAOImp implements IStudentDAO {
	...
	public StudentInfo getStudentById(int id) {
		...
		try {
			...
			if(rs.next()){
				...
				stu.setGradeFrom(rs.getString("grandefrom"));
			}
	...
	public List<StudentInfo> getStudentsByName(String name) {
		...
		ResultSet rs = null;
		try {
			//获取当前页码的游标， 每页几行，当前页码，索引的名字，当前名字的个数
			...
			try {
				...
				prepareCall.execute();
				//设置查询到的所有数据的个数
				//PageManager.setMaxRows(prepareCall.getInt(5));
				PageManager.maxRows=prepareCall.getInt(5);
				//初始化
				if(PageManager.initFlg==false){
					PageManager.initPageInfo(8, 1, prepareCall.getInt(5));
					PageManager.initFlg=true;
				}
